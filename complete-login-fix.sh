...
# The password '<LERA_SEED_ADMIN_PASSWORD>' hashed with BCrypt
# This is a known valid BCrypt hash for '<LERA_SEED_ADMIN_PASSWORD>'
psql -h localhost -U lera -d lera << 'EOF'
-- Delete existing admin if any
DELETE FROM users WHERE email = 'admin@lera.com';

-- Insert admin with proper BCrypt hash for password '<LERA_SEED_ADMIN_PASSWORD>'
INSERT INTO users (
  id,
  center_id,
  role_id,
