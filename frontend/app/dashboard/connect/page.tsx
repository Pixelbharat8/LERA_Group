...
const RTC_PEER_CFG: RTCConfiguration = {
...
type ResolvedConversationLabels = {
  displayName: string;
  roleLabel: string | null;
  otherParticipantId?: string;
};

const SYNTHETIC_USER_NAME = /^User [a-f0-9-]{8,}$/i;

type MediaPermissionKind = "voice" | "video";
type MediaPermissionIssue = "unsupported" | "insecure" | "denied" | "not_found" | "unknown";

function getMediaPermissionIssue(error: unknown): MediaPermissionIssue {
...
function mediaPermissionMessage(kind: MediaPermissionKind, issue: MediaPermissionIssue): string {
...
function formatConversationName(displayName: string, roleLabel: string | null): string {
...
type CurrentUserLike = Pick<User, "id" | "fullname" | "name" | "email"> | null;
...
function normalizeSavedChatMessage(saved: unknown): Record<string, unknown> {
