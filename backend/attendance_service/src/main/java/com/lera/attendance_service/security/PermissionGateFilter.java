 ...
 *   1. ROLE layer — role_permissions table (granular codes, e.g. attendance.mark).
 ...
 * Unmapped endpoints pass through (role-based @PreAuthorize still applies). /api/internal/** is never mapped.
 */
@Component
public class PermissionGateFilter extends OncePerRequestFilter {
    ...
    /** Attendance endpoints are governed by the Chairman's "attendance" toggle. */
    private String userPermissionColumn(String path) {
        return requiredPermission("GET", path) != null ? "attendance_access" : null;
    }
