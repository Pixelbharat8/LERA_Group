...
public class PermissionGateFilter extends OncePerRequestFilter {
    ...
    private String requiredPermission(String method, String path) {
        String base;
        if (matches(path, "/api/students") || matches(path, "/api/enrollments")) {
            base = "students";
        } else if (matches(path, "/api/teachers")) {
            base = "teachers";
        } else if (matches(path, "/api/classes") || matches(path, "/api/courses")) {
            base = "classes";
        } else {
            return null; // exams, grades, library, cms, calendar … stay role-only
        }
